document.addEventListener("DOMContentLoaded", function() {
    ...
    function inicializarEventListeners() {
        ...
        document.querySelectorAll('.impressora-item').forEach(option => {
            option.addEventListener('click', function() {
                const printerId = this.getAttribute('data-id'); // Corrigido para data-id
                document.querySelectorAll('.impressora-item').forEach(opt => opt.classList.remove('selected'));
                this.classList.add('selected');
                // Não há um input 'impressora' visível na tela principal, a seleção é visual
                // Se precisar armazenar o ID selecionado para o cálculo, pode usar uma variável global ou um input hidden
                // Exemplo: document.getElementById('impressora_selecionada_id').value = printerId;
            });
        });
